...
function SettlementPanel({
  ...
  superblock: XtDetail['superblock'] | null;
  chains: Map<number, ChainView>;
      ...
      <div className="mini-list">
        {superblock ? (
          <>
            <FieldRow
              label="Superblock"
              value={`#${superblock.number} (${superblock.status})`}
            />
            {superblock.gameAddress && (
              <FieldRow
                label="Dispute Game"
                value={shortHex(superblock.gameAddress, 8, 6)}
                copy={superblock.gameAddress}
              />
            )}
            {superblock.l1Tx && (
              <FieldRow label="L1 Tx" value={shortHex(superblock.l1Tx, 8, 6)} copy={superblock.l1Tx} />
            )}
            {superblock.l1TxFee && <FieldRow label="L1 Fee" value={formatFee(superblock.l1TxFee)} />}
            {superblock.chains.map((block) => {
            ...
            <Button variant="subtle" size="sm" onClick={() => onSuperblock(superblock!.number)}>
              View superblock
            </Button>
          </>
        ) : (
...
function OverviewTab({
  ...
  chains: Map<number, ChainView>;
  mailbox: MailboxMessage[];
  transfers: XtDetail['transfers'];
  tokens: XtDetail['tokens'];
  participants: number[];
  decision: string;
  deliveryMsg: MailboxMessage | undefined;
  onSuperblock: (number: number) => void;
  superblock: XtDetail['superblock'] | null;
}) {
  return (
    <>
      ...
      <div className="two-col">
        ...
        <div className="stack">
          <CoordinationPanel participants={participants} mailbox={mailbox} decision={decision} chains={chains} />
          <SettlementPanel superblock={superblock} chains={chains} onSuperblock={onSuperblock} />
        </div>
      </div>
      <section className="panel panel-spaced">
    ...
    <>
      ...
      {tab === 'overview' && (
        <OverviewTab
          ...
          onSuperblock={onSuperblock}
          superblock={detail?.superblock ?? null}
        />
      )}
